...
public class Objeyaraticisi : MonoBehaviour {
	...
	float z_ust;
	float z_alt;
	float dilimgenisligi = 0.025f; //dilimlerin genişliği
	...
	void Update () {
			...
			z_ust = Mathf.Sqrt (1.25f*1.25f - (xcenter -1.0f - dilimgenisligi/2) * (xcenter -1.0f - dilimgenisligi/2))+1;
			z_ust = -1 * z_ust;

			z_alt = Mathf.Sqrt (0.75f*0.75f - (xcenter -1.0f + dilimgenisligi/2) * (xcenter -1.0f + dilimgenisligi/2))+1;
			z_alt = -1 * z_alt;
			z = (float) (z_ust - z_alt);
			...
			bc.size = new Vector3(dilimgenisligi,y,z);




			xcenter = xcenter - dilimgenisligi;
